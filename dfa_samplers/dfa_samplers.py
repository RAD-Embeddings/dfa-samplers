...
class DFASampler:
    def __init__(self, n_tokens: int = 10, min_size: int = 3, max_size: int = 10):
        assert n_tokens > 1 and max_size >= 2
        self.n_tokens = n_tokens
        self.min_size = min_size
        self.max_size = max_size
        assert self.min_size >= 2 and self.max_size >= self.min_size
...
class ReachSampler(DFASampler):
    ...
    def _sample(self) -> DFA:
        ...
        n = np.random.randint(self.min_size - 1, self.max_size) + 1
        success = n - 1
        transitions = {
...
class ReachAvoidSampler(DFASampler):
    def __init__(self, prob_stutter: float = 0.9, p: float | None = None, **kwargs):
        super().__init__(**kwargs)
        assert 0 <= prob_stutter <= 1
        assert self.min_size >= 3
        self.prob_stutter = prob_stutter
        self.p = p
        self.n_values = np.array(list(range(self.min_size - 2, self.max_size - 2 + 1)))
...
class RADSampler(DFASampler):
    ...
    def _sample_rad(self) -> DFA:
        candidate = self.reach_avoid_sampler.sample()
        for i in range(np.random.choice(self.max_mutations + 1)):
    ...
    def _sample(self) -> DFA:
        candidate = self._sample_rad()
        n_states = len(candidate.states())
        while n_states < self.min_size or n_states > self.max_size:
            candidate = self._sample_rad()
            n_states = len(candidate.states())
        return candidate